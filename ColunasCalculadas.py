...
def statusVencimentoT2(tipotransporte, data_hoje, data_limite):
        try:
            if tipotransporte == "Direto":
                return blank
            elif pd.isnull(data_limite):
                return "00. Sem Data"
            else:
                d_limite = pd.to_datetime(data_limite, infer_datetime_format=True, dayfirst=True)
                d_hoje = pd.to_datetime(data_hoje, infer_datetime_format=True, dayfirst=True)
...
def statusVencimentoparc(data_hoje, data_limite):
    try:
        if pd.isnull(data_limite):
            return "00. Sem Data"
        else:
        ...
        #    return "02. Atraso Ontem"
            ...
            return "04. Vence Semana"
        elif (weekday_hoje >= 4 or weekday_hoje == 0) and weekday_limite == 1 and semana_d_limite == (
                semana_d_hoje + 1):
            return "05. Vence Segunda"
        elif semana_d_limite == 1 and semana_d_hoje == 52:  ## feito para virada de ano
            return "06. Vence S+1"
...
def fasenota(pos_final, origem, p1, arquivo, d42, edi_44):
...
def stepparceiro(edi44, d53, d54, d55, d56, d57):
    edi = edi44
    if pd.isnull(d56) == False:
        step_parceiro = "ENTREGUE PARCEIRO"

    elif pd.isnull(d57) == False:
        step_parceiro = "ENTREGUE PARCEIRO"

    elif edi == "Chegada Destino" or edi == "Mercadoria Entregue no Parceiro" or edi == "Devolução Autorizada Mi" or edi == "Entrega Prejudicada por Horário/Falta de Tempo Hábil" or edi == "Entregue por outra transportadora" or edi == "Em rota de Entrega":
        step_parceiro = "ENTREGUE PARCEIRO"
...
def faseatraso(a_fornecedor, a_coleta, a_cd_ff, a_next, a_transp, reprogramado):
    if a_fornecedor == 1:
...
def diario_flag(arquivo, atraso_MM, atraso_cliente, atraso_tp, atraso_redespacho, agT3, agCliente, agTransporte, agedi,
    ...
    if arquivo == "Tableau" or arquivo == "VS":
...
def transpLMFinal(tplm, tpparceiro, tp_lmv2):
    if tp_lmv2 == "FULFILLMENT - NEXT DAY":
        return "BULKY-JUNDIAI"
    elif tplm == 'BULKYLOG - UBA':
        return tpparceiro
    elif tplm == "RECIFE - BULKY" and tpparceiro != blank_str:
        return tpparceiro
    elif tplm == "SJRP - BULKYLOG" and tpparceiro != blank_str:
        return tpparceiro
    elif tplm == "ITAJAI-BULKY" and tpparceiro != blank_str:
        return tpparceiro
        ...
        if tpparceiro == "ARAPON-BULKY - P":
            ...
            return "ITAJAI-BULKY"
        elif tpparceiro == "KUWABATA SANTO ANTONIO":
            return "KUWABATA SANTO ANTONIO"
        elif tpparceiro == "KUWABATA MARINGA":
            return "KUWABATA MARINGA"
        elif tpparceiro != blank_str:
            return tpparceiro
        else:
